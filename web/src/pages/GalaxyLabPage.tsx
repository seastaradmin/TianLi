...
import type { HeroState, Language, SkySnapshot, SkillDispatchState, TaskState, UiAnchor } from '../types'
import { apiUrl } from '../utils/api'

const SSE_URL = apiUrl('/api/logs/stream')
...
export default function GalaxyLabPage() {
  ...
  const syncSkySnapshot = useCallback(async () => {
    const snapshot = await fetchJson<SkySnapshot>(apiUrl('/api/status'))
    hydrateSky(snapshot)
    hydrateStats(snapshot.stats)
    return snapshot
  ...
  useEffect(() => {
    ...
    void loadSnapshot()
  }, [addLog, addLogs, clearLogs, hydrateSky, hydrateStats, language])
  ...
  const handleIssueDestiny = async () => {
    ...
    try {
      const response = await fetchJson<{ taskId: string }>(apiUrl('/api/run/start'), {
        method: 'POST',
      ...
      try {
        ...
        console.warn('[GalaxyLabPage] failed to sync sky snapshot after issuing destiny', snapshotError)
      }
  ...
  const handleVerdict = async (verdict: 'approve' | 'reject') => {
    ...
    setIsSubmitting(true)
    try {
      await fetchJson(apiUrl('/api/run/verdict'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleRefreshSkills = async () => {
    try {
      await fetchJson(apiUrl('/api/skills/refresh'), { method: 'POST' })
      addLog({
        id: `refresh-${Date.now()}`,
        time: new Date().toLocaleTimeString(language === 'zh' ? 'zh-CN' : 'en-US'),
        level: 'INFO',
